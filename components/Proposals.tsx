...
const iconMap: Record<string, React.ReactNode> = {
  'Eventos': <Calendar className="w-8 h-8 text-red-600" />,
  'Gestão': <FileText className="w-8 h-8 text-red-600" />,
  'Pessoas': <MessageSquare className="w-8 h-8 text-red-600" />,
  'Estrutura': <Wrench className="w-8 h-8 text-red-600" />,
  'Segurança': <Lock className="w-8 h-8 text-red-600" />,
  'Esportes': <TrendingUp className="w-8 h-8 text-red-600" />,
  'default': <Package className="w-8 h-8 text-red-600" />
};
    ...
    <section id="propostas" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl lg:text-5xl font-black text-red-950 mb-4">Plano de Modernização AESJ</h2>
                ...
                <div className="p-10 h-full flex flex-col">
                  <div className="flex justify-between items-start mb-8 transition-transform group-hover:scale-[1.02]">
                    <div className="p-4 bg-white/10 backdrop-blur-md rounded-2xl w-fit shadow-md border border-white/10">
                      ...
                      {React.cloneElement(iconMap[proposal.category] as React.ReactElement, { className: "w-8 h-8 text-red-400" })}
                    </div>
                    <div className="flex flex-col items-end gap-2 text-right">
                      <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${
                        proposal.status === 'Concluído' ? 'bg-green-500/20 text-green-400 border border-green-500/30' :
                        proposal.status === 'Executando' ? 'bg-red-500/20 text-red-400 border border-red-500/30' :
                  ...
                  <h3 className="text-3xl font-black text-white mb-4 flex items-center gap-4">
                    <span className="text-red-500/30 text-5xl font-black leading-none">
                  ...
                  {(proposal.goal || proposal.eta) && (
                    <div className="space-y-6 pt-8 border-t border-white/10">
                      {proposal.goal && (
                        <div>
                          <h4 className="text-xs font-black text-red-400 uppercase tracking-widest mb-2">🎯 Objetivo</h4>
                      ...
                      {proposal.eta && (
                        <div className="flex justify-between items-end">
                          <div>
                            <h4 className="text-xs font-black text-red-400 uppercase tracking-widest mb-2">⏳ Prazo Estimado</h4>
                            <p className="text-sm text-gray-300 font-bold">{proposal.eta}</p>
                          </div>
                          <button className="text-red-400 font-black text-xs uppercase tracking-widest hover:text-white transition-all">
                            Ver Detalhes +
                          </button>
                        </div>
                      )}
        ...
        <div className="mt-20 p-12 bg-red-900 rounded-[3rem] text-center text-white relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-red-700/30 rounded-full -mr-32 -mt-32 blur-3xl" />
